...
export default async function Restaurants() {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/content/restaurants`, {
      cache: 'no-store'
    });
    if (response.ok) {
    ...
    console.log('Error fetching restaurants content from API:', error);
  }
  ...
  if (!content) {
    try {
      ...
      const contentPath = path.join(process.cwd(), 'src', 'content', 'restaurants.json');
      const fileContent = fs.readFileSync(contentPath, 'utf8');
      content = JSON.parse(fileContent);
    } catch (error) {
      console.log('Error loading restaurants content from file:', error);
    }
  }
