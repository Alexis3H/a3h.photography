...
export default async function Evenements() {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/content/evenements`, {
      cache: 'no-store'
    });
    if (response.ok) {
    ...
    console.log('Error fetching evenements content from API:', error);
  }
  ...
  if (!content) {
    try {
      ...
      const contentPath = path.join(process.cwd(), 'src', 'content', 'evenements.json');
      const fileContent = fs.readFileSync(contentPath, 'utf8');
      content = JSON.parse(fileContent);
    } catch (error) {
      console.log('Error loading evenements content from file:', error);
    }
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      {content.sections.social && (
        <section id="social" className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-8">
          ...
          <div className="md:w-1/2">
            <Image
              src={content.sections.social.image?.src || '/uploads/_DSC4541.jpg'}
